...
class Model: 
        ...
        def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False):
                ...
                with tf.control_dependencies(self.update_ops):
                    # TODO: use the right kind of optimizer
        ...
        def clipped_relu(self, inputs):
            return tf.clip_by_value(tf.nn.relu(inputs), tf.constant(0.), tf.constant(20.))

        def setupCNN_old(self):
                ...
                # TODO: 3 Layer1D
                # 512, 512, 512 channels, 5, 5, 5, filter, 1, 1, 2 stride
                # I think each channel is a spectrogram bucket?
                channels1D = [Model.imgSize[1], 512, 512, 512]
                filters1D = [5, 5, 5]
                strides1D = [1, 1, 2]
                featureVals1D = [Model.imgSize[1]] * 4

                channels2D = [32, 32, 96]
                filters2D = [(41, 11), (21, 11), (21, 11)]
                strides2D = [(2, 2), (2, 1), (2, 1)]
                numLayers = 3
                out = self.inputImgs
                for i in range(numLayers):
                    kernel = tf.Variable(tf.truncated_normal([filters1D[i], featureVals1D[i], featureVals1D[i + 1]], stddev=0.1))
                    out = tf.nn.conv1d(out, kernel, padding='SAME', stride=strides1D[i])
